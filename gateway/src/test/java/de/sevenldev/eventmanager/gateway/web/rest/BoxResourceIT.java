...
import de.sevenldev.eventmanager.gateway.repository.BoxRepository;
import de.sevenldev.eventmanager.gateway.repository.search.BoxSearchRepository;
...
public class BoxResourceIT {
     ...
     *
     * @see de.sevenldev.eventmanager.gateway.repository.search.BoxSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void getAllBoxesWithEagerRelationshipsIsEnabled() throws Exception {
        BoxResource boxResource = new BoxResource(boxRepositoryMock, mockBoxSearchRepository);
        when(boxRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restBoxMockMvc = MockMvcBuilders.standaloneSetup(boxResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restBoxMockMvc.perform(get("/api/boxes?eagerload=true"))
        .andExpect(status().isOk());

        verify(boxRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllBoxesWithEagerRelationshipsIsNotEnabled() throws Exception {
        BoxResource boxResource = new BoxResource(boxRepositoryMock, mockBoxSearchRepository);
            when(boxRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restBoxMockMvc = MockMvcBuilders.standaloneSetup(boxResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
