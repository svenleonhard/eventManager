...
/**
 * REST controller for managing {@link de.sevenldev.eventmanager.gateway.domain.Box}.
 */
@RestController
@RequestMapping("/api")
public class BoxResource {
     ...
     */
    @GetMapping("/boxes")
    public ResponseEntity<List<Box>> getAllBoxes(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Boxes");
        Page<Box> page;
        if (eagerload) {
            page = boxRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = boxRepository.findAll(pageable);
        }
    ...
    public ResponseEntity<Box> getBox(@PathVariable Long id) {
        ...
        Optional<Box> box = boxRepository.findOneWithEagerRelationships(id);
